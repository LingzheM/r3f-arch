...
| # | 问题 | 性质 | 优先级 | 到期 | 状态 |
|---|---|---|---|---|---|
| P1 | 多层叠放时，拖动墙 / 放门窗会作用到**下层**的墙 | **bug** | **高** | **M9 之前** | 修法在 `m9-persistence.html` 前置 C · C2（沙箱验过）。**原方案漏一行，见 P1 末尾更正** |
| P2 | 创建天花板后，天花板**选不中** | 待查 | **高** | **M9 之前** | 有假设，待复现（2026-09-15 用户还没看状态栏） |
| P3 | 屋顶层创建天花板，天花板**悬空** | 设计如此 + 演示文档写错 | 中 | M9 之前（改文档） | 演示步骤更正版在前置 C · C5 |
| P4 | 顶视图放门窗，门窗**朝向无法选择** | bug | 中 | M9 之前 | 修法在前置 C · C3（`sideFromHit`，沙箱验过 + three 射线探针 + 变异测试） |
| P5 | 楼板（多边形）预览虚线被墙遮住 | 体验 | 中 | M9 之前 | 修法在前置 C · C4（**待浏览器确认**） |
...
> **更正（2026-09-15，M9 前置会话，按工作约定第 6 条）：上面的方案漏了一行。**
> R3F 的 `stopPropagation` 在被拦的物体正处于悬停状态时，会调 `cancelPointer([...更近的, 自己])`
> （`@react-three/fiber` 9.7.0 `events-b1bdeb1a.cjs.dev.js:782-785`），
> 给**射线上更远、之前悬停过**的物体补发 `onPointerLeave`。`OpeningTool.onWallLeave` 不看是谁离开，
> 于是下层墙的 `wall:leave` 会把刚设好的预览清空（从只打中下层墙的角度移进重叠区时触发）。
> 修法：`onWallLeave` 只在 `previewRef.current?.wallId === e.node.id` 时清空。全文在 `m9-persistence.html` 前置 C · C2。
